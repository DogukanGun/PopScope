...
class CountryService:
    def __init__(self):
        ...
        countries_df = self.df[~self.df['Country Code'].isin(exclude_codes)].copy()
        self.countries = [
            {
                'country_name': record['Country Name'],
                'country_code': record['Country Code']
            }
            for record in countries_df[['Country Name', 'Country Code']].dropna().to_dict('records')
        ]

    def get_countries(self) -> List[Dict]:
        return self.countries

    def get_population_data(self, country_codes: List[str]) -> List[Dict]:
